...
DRV8833::DRV8833(uint_least8_t
    ...
    GPIODirModeSet(PORT, AIN1_BIT, GPIO_DIR_MODE_OUT);
    GPIODirModeSet(PORT, AIN2_BIT, GPIO_DIR_MODE_OUT);
    GPIODirModeSet(PORT, BIN1_BIT, GPIO_DIR_MODE_OUT);
    GPIODirModeSet(PORT, BIN2_BIT, GPIO_DIR_MODE_OUT);
}
...
void DRV8833::stepMotor(int32_t step) {
    ...
    switch (step) {
        case 0: // 1010
            a_mask = GPIOPinRead(PORT, 0xFF) & ~AIN2_BIT | AIN1_BIT;
            b_mask = GPIOPinRead(PORT, 0xFF) & ~BIN2_BIT | BIN1_BIT;
            GPIOPinWrite(PORT, AIN1_BIT | AIN2_BIT, a_mask);
            GPIOPinWrite(PORT, BIN1_BIT | BIN2_BIT, b_mask);
            break;
        case 1: // 0110
            a_mask = GPIOPinRead(PORT, 0xFF) & ~AIN1_BIT | AIN2_BIT;
            b_mask = GPIOPinRead(PORT, 0xFF) & ~BIN2_BIT | BIN1_BIT;
            GPIOPinWrite(PORT, AIN1_BIT | AIN2_BIT, a_mask);
            GPIOPinWrite(PORT, BIN1_BIT | BIN2_BIT, b_mask);
            break;
        case 2: // 0101
            a_mask = GPIOPinRead(PORT, 0xFF) & ~AIN1_BIT | AIN2_BIT;
            b_mask = GPIOPinRead(PORT, 0xFF) & ~BIN1_BIT | BIN2_BIT;
            GPIOPinWrite(PORT, AIN1_BIT | AIN2_BIT, a_mask);
            GPIOPinWrite(PORT, BIN1_BIT | BIN2_BIT, b_mask);
            break;
        case 3: // 1001
